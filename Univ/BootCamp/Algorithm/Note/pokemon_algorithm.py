...
if __name__ == "__main__":
    ...
    po_owner = list(counts.keys())
    po_name = list(counts.values())
    ...
    print("===정렬 전===")
    for i in range(0, len(po_owner)):
        print(f'{po_owner[i]} 소유 포켓몬은 총 {counts[po_owner[i]]}마리 입니다.')

    my_sort(po_name, po_owner, 0, len(counts) - 1)
    print("===정렬 후===")
    for i in range(0, len(po_owner)):
        print(f'{po_owner[i]} 소유 포켓몬은 총 {counts[po_owner[i]]}마리 입니다.')
